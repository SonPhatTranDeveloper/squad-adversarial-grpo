import torch

from src.utils.inference import GRPOInference


def main() -> int:
    # Define model and adapter
    model_id = "Qwen/Qwen2.5-3B-Instruct"
    adapter_path = "outputs/qwen2.5-3b-grpo/checkpoint-2000"
    device_map = "cuda"
    dtype = torch.bfloat16
    max_new_tokens = 1024
    temperature = 1.0
    top_p = 0.9
    do_sample = True

    # Define a single context and a list of questions
    context = "Paris is the capital of France."
    questions = [
        "What is the capital of France?",
        "Which city is the capital of France?",
        "Name France's capital city.",
    ]

    # Repeat the same context for each question
    contexts = [context] * len(questions)

    # Define inference
    infer = GRPOInference(
        model_id=model_id,
        adapter_path=adapter_path,
        device_map=device_map,
        dtype=dtype,
        max_new_tokens=max_new_tokens,
        temperature=temperature,
        top_p=top_p,
        do_sample=do_sample,
    )

    # Batch-generate adversarial sentences
    results = infer.generate_many(contexts=contexts, questions=questions, batch_size=3)
    answers = [item["answer"] for item in results]

    for q, a in zip(questions, answers, strict=False):
        print(f"Q: {q}\nA: {a}\n")


if __name__ == "__main__":
    main()
